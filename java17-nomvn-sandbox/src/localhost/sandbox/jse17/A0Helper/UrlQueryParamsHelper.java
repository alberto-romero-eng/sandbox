...
public class UrlQueryParamsHelper {
	...
	public static void main() {
		System.out.println("Hello from UrlQueryParamsHelper!");
		test00();
	...
	public static String toUrlQueryString(Map<String,String> urlQueryParams) {
		String urlQueryString = urlQueryParams.keySet().stream()
				.map( (key) -> {
					...
					try {
						encodedValue = URLEncoder.encode(urlQueryParams.get(key), StandardCharsets.UTF_8); // JavaSE-8: StandardCharsets.UTF_8.name()
					} catch (Throwable ex) {
