...
__KH_REGEX_PRO_PIT__='pit'
__KH_REGEX_V2_PRO_COMMON_TOOL__='v2-pro-common-tool'
...
__KH_SOURCE_PORT_KEEPA__=10033
...
function __kh_load {
    ...
    __kh_service_keepa=$( echo "${service_list}" | command grep -P -i ${__KH_REGEX_KEEPA__} | command awk '{print $1}' )
    ...
    __kh_deployment_keepa=$( echo "${deployment_list}" | command grep -P -i ${__KH_REGEX_KEEPA__} | command awk '{print $1}' )
    echo 'done'
    ...
    __kh_pod_keepa=$( echo "${pod_list}" | command grep -P -i ${__KH_REGEX_KEEPA__} | command awk '{print $1}' )
    echo 'done'
    ...
    __kh_container_keepa=${__kh_deployment_keepa}
    ...
    __kh_target_port_keepa=$( echo "${service_list}" | command grep -P -i ${__KH_REGEX_KEEPA__} | command awk '{print $5}' | sed '{ s/\/TCP$//ig }' )
    ...
    s=${s}'__kh_service_keepa='${__kh_service_keepa}' \n'
    ...
    s=${s}'__kh_deployment_keepa='${__kh_deployment_keepa}' \n'
    s=${s}'\n'
    s=${s}'# PODS \n'
    ...
    s=${s}'__kh_pod_keepa='${__kh_pod_keepa}' \n'
    ...
    s=${s}'__kh_container_keepa='${__kh_container_keepa}' \n'
    ...
    s=${s}'__KH_SOURCE_PORT_KEEPA__='${__KH_SOURCE_PORT_KEEPA__}' \n'
    ...
    s=${s}'__kh_target_port_keepa='${__kh_target_port_keepa}' \n'
...
function __kh_set_service {
    ...
    if [ ${__kh_service} = ${__kh_service_api} ] ; then
    ...
    elif [ ${__kh_service} = ${__kh_service_keepa} ] ; then
        __kh_deployment=${__kh_deployment_keepa}
        __kh_pod=${__kh_pod_keepa}
        __kh_container=${__kh_container_keepa}
        # __kh_service=${__kh_service_keepa}
        __kh_source_port=${__KH_SOURCE_PORT_KEEPA__}
        __kh_target_port=${__kh_target_port_keepa}
        ...
        __kh_source_port=10298
        __kh_target_port=$( echo "${service_list}" | command grep -P -i ${__regex} | command awk '{print $5}' | sed '{ s/\/TCP$//ig }' )
    fi
    __kh_print_vars
}
...
function __kh_log {
    ...
    # kubectl log 
    _s=${kk}' logs '${__kh_pod}' '${__kh_container}' '${_tflag}' '${_fflag}' &> '\''./tmp/'${__kh_container}'.log'\'' & '
    echo 'executing: '
    echo ${_s}
    eval ${_s}
...
function __kh_get_namespaces {
    ${kk} get namespaces > ./tmp/get-namespaces.txt
    cat ./tmp/get-namespaces.txt
}




function __kh_describe_namespace {
    if [ -z ${__kh_namespace} ] ; then
        echo 'variable ${__kh_namespace} must be defined -- exit'
        return 1;
    fi
    ${kk} describe namespace ${__kh_namespace} > ./tmp/describe-namespace.txt
    cat ./tmp/describe-namespace.txt
}
...
function __kh_zerohelper_init {
    # cdkk_s
    command source ./${__KH_THIS_FILE__}
...
alias 19='  __kh_load '${__KH_REGEX_PRO_PIT__}' ; '
alias 110=' __kh_load '${__KH_REGEX_V2_PRO_COMMON_TOOL__}' ; '
...
alias 233=' __kh_set_service '${__KH_REGEX_KEEPA__}' ; '
...
alias 96='  __kh_get_namespaces ; '
alias 97='  __kh_describe_namespace ; '
